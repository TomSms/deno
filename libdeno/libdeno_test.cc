...
TEST(LibDenoTest, Snapshotter) {
  ...
  Deno* d2 = deno_new(test_snapshot, deno_config{empty, nullptr, nullptr});
  EXPECT_TRUE(
...
TEST(LibDenoTest, Shared) {
  ...
  Deno* d = deno_new(snapshot, deno_config{shared, nullptr, nullptr});
...
TEST(LibDenoTest, ModuleResolution) {
  ...
  auto resolve_cb = [](void* user_data, const char* specifier,
    ...
    count++;
    const char* src_b = "export function retb() { return 'b'; }";
    auto d = reinterpret_cast<Deno*>(user_data);
    deno_resolve_ok(d, "b.js", src_b);
  };
  Deno* d = deno_new(empty, deno_config{empty, nullptr, resolve_cb});
  EXPECT_TRUE(deno_execute(d, d, "a.js", src_a));
  EXPECT_EQ(count, 1);
  deno_delete(d);
}
...
TEST(LibDenoTest, ModuleResolutionFail) {
  ...
  Deno* d = deno_new(empty, deno_config{empty, nullptr, resolve_cb});
  EXPECT_FALSE(deno_execute(d, d, "a.js", src_a));
  EXPECT_EQ(count, 1);
  deno_delete(d);
}
